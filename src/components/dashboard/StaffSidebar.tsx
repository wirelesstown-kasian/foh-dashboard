...
function getLatestClockRecord(records: ShiftClock[], employeeId: string) {
  return [...records]
...
function getRelatedEmployee(record: ShiftClock | null, employees: Employee[], employeeId: string) {
  const relatedEmployee = record?.employee as Employee | Employee[] | undefined
  return employees.find(item => item.id === employeeId) ?? (Array.isArray(relatedEmployee) ? relatedEmployee[0] : relatedEmployee) ?? null
}
...
export function StaffSidebar({ schedules, employees, clockRecords }: Props) {
  ...
  const staffOnToday = useMemo(() => {
    const scheduledEntries = schedules.map((schedule): StaffEntry | null => {
      ...
      const record = getLatestClockRecord(clockRecords, schedule.employee_id)
    ...
    const clockOnlyEntries = Array.from(new Set(
      ...
      const record = getLatestClockRecord(clockRecords, employeeId)
      const employee = getRelatedEmployee(record, employees, employeeId)
      if (!employee) return null
      return {
        employee,
        schedule: null,
  ...
  const groupedStaff = useMemo(() => {
    ...
    const presentDepartmentKeys = Array.from(new Set(staffOnToday.map(entry => entry.department).filter(Boolean)))
    ...
    return departmentKeys
      .map(department => ({
        department,
        label: getDepartmentLabel(department, departmentDefinitions),
        entries: staffOnToday
  ...
  return (
    <aside className="flex min-h-0 w-72 shrink-0 flex-col border-r bg-white">
      ...
      <div className="min-h-0 flex-1 space-y-4 overflow-y-auto p-3">
        ...
        {groupedStaff.map(({ department, label, entries }, index) => {
          const isOpen = openDepartments[department] ?? (entries.length > 0 || index === 0)
                    ...
                    return (
                      <div key={`${employee.id}-${schedule?.id ?? 'clock'}`} className="rounded-lg border border-slate-200 bg-slate-50 px-2 py-1.5">
                        ...
                        <div className="mt-0.5 flex items-center justify-between gap-1">
                          <span className="text-[10px] uppercase tracking-wide text-slate-400">{getRoleLabel(employee.role, roleDefinitions)}</span>
                          {employee.phone && (
